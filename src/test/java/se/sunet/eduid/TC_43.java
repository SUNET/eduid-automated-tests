...
public class TC_43 {
    ...
    void register(){ register.runRegister(acceptTerms, incorrectMagicCode, true); }

    @Test( dependsOnMethods = {"register"} )
    void confirmHuman() { confirmHuman.runConfirmHuman(captcha, incorrectMagicCode, true); }

    @Test( dependsOnMethods = {"confirmHuman"} )
    void confirmedNewAccount() { confirmedNewAccount.runConfirmedNewAccount(); }

    @Test( dependsOnMethods = {"confirmedNewAccount"} )
    void login(){ login.runLogin(common.getUsername(), common.getRecommendedPw(), resetPassword, false, incorrectPassword); }
    ...
    void startPage2(){ startPage.runStartPage(registerAccount); }

    @Test( dependsOnMethods = {"startPage2"} )
    void register2(){ register.runRegister(acceptTerms, incorrectMagicCode, generateUsername); }

    @Test( dependsOnMethods = {"register2"} )
    void confirmHuman2() { confirmHuman.runConfirmHuman(captcha, incorrectMagicCode, generateUsername); }
